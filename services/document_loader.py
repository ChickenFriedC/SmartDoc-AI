...
def load_pdf(file_path: str, filename: str) -> List[Document]:
    ...
    for page_idx, doc in enumerate(docs, start=1):
        ...
        cleaned_docs.append(Document(page_content=text, metadata={"source": file_path, "filename": filename, "source_type": "PDF", "page": page_idx, "uploaded_at": datetime.now().isoformat()}))
    return cleaned_docs
...
def load_docx(file_path: str, filename: str) -> List[Document]:
    ...
    for idx, para in enumerate(docx_file.paragraphs, start=1):
        ...
        if not text: continue
        docs.append(Document(page_content=text, metadata={"source": file_path, "filename": filename, "source_type": "DOCX", "paragraph_index": idx, "uploaded_at": datetime.now().isoformat()}))
    return docs
...
def process_uploaded_files(uploaded_files, chunk_size: int, chunk_overlap: int):
    ...
    def _process_single_file(uploaded_file):
        ...
        with tempfile.NamedTemporaryFile(delete=False, suffix=suffix) as tmp:
            ...
            temp_path = tmp.name
        try:
            if suffix == ".pdf": raw_docs = load_pdf(temp_path, uploaded_file.name)
            elif suffix == ".docx": raw_docs = load_docx(temp_path, uploaded_file.name)
            else: return None, None
            return raw_docs, {"filename": uploaded_file.name, "type": suffix.replace('.', '').upper(), "raw_units": len(raw_docs)}
        finally:
            if os.path.exists(temp_path): os.remove(temp_path)

    # Đa luồng xử lý file
    with ThreadPoolExecutor() as executor:
        results = list(executor.map(_process_single_file, uploaded_files))
    
    for docs, summary in results:
        if docs:
